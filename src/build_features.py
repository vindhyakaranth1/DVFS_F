...
INPUT_PATH = os.path.join(PROJECT_ROOT, "data", "cpu_log_prepared.csv")
OUTPUT_X = os.path.join(PROJECT_ROOT, "data", "X_features.npy")
OUTPUT_Y = os.path.join(PROJECT_ROOT, "data", "y_labels.npy")
...
for i in range(WINDOW, len(cpu_vals)):
    ...
    X.append(features)
    y.append(cpu_vals[i])  # next-step CPU util (normalized)
